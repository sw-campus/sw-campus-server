...
public interface ReviewJpaRepository extends JpaRepository<ReviewEntity, Long> {
        ...
        @Query("""
                        SELECT r FROM ReviewEntity r LEFT JOIN FETCH r.details
        ...
        @Query("""
                        SELECT r.lectureId, AVG(r.score) FROM ReviewEntity r
        ...
        @Query("""
                        SELECT r.lectureId, COUNT(r) FROM ReviewEntity r
                        WHERE r.lectureId IN :lectureIds AND r.approvalStatus = :status
                        GROUP BY r.lectureId
                        """)
         ...
         */
        @Query("""
                        SELECT r.lectureId, COALESCE(AVG(r.score), 0.0), COUNT(r)
                        FROM ReviewEntity r
                        WHERE r.lectureId IN :lectureIds AND r.approvalStatus = :status
                        GROUP BY r.lectureId
                        """)
